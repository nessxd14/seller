...
export function CustomersPage({ notify }: { notify: (message: string) => void }) {
  ...
  const filtered = useMemo(() => customers.filter((customer) => coincideBusqueda(`${customer.name} ${customer.document} ${customer.email}`, query)), [customers, query])
...
function CustomerEditor({ customer, isNew, onClose, onSave, onGoToCustomer }: { customer: CustomerRecord; isNew: boolean; onClose: () => void; onSave: (value: CustomerRecord) => void | Promise<void>; onGoToCustomer: (customer: CustomerRecord) => void }) {
  ...
  const submit = async () => {
    ...
    setDuplicado(null)
    try {
      await onSave({ ...value, document: value.document.trim() })
    } catch (err) {
  ...
  return <Modal title={isNew ? 'Nuevo cliente' : 'Editar cliente'} onClose={onClose} wide><div className="modal-body form-grid">
    ...
    {error && <div className="full field-error">
      <p>{error}</p>
