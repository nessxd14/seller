...
function CloseCashModal({session,expected,byMethod,onClose,onConfirm}:{session:CashSessionRecord;expected:number;byMethod:Record<string,number>;onClose:()=>void;onConfirm:(amount:number)=>void}) {
  ...
  </div><footer className="modal-actions"><button className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" onClick={()=>confirm(featureFlags.supabase ? '¿Confirmar cierre de caja?' : '¿Confirmar cierre de caja mock?')&&onConfirm(counted)}>Confirmar cierre</button></footer></Modal>
}
