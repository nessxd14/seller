...
export function SuspendedSalesPage({ hasCurrentCart, onRestore, notify }:{hasCurrentCart:boolean;onRestore:(sale:SuspendedSale)=>void;notify:(message:string)=>void}) {
  const [sales,setSales]=useState(readSuspended)
  ...
  const remove=(id:string)=>{if(!confirm('¿Eliminar definitivamente esta venta suspendida local?'))return;const next=sales.filter((sale)=>sale.id!==id);localStorage.setItem('roari-suspended-sales-v2',JSON.stringify(next));setSales(next);notify('Venta suspendida eliminada')}
